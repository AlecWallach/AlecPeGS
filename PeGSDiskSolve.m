...
files = dir(fullfile(directory, 'DSC03393_preprocessing.mat')); 
...
for frame = 1:nFrames %loop over these frames 
    fileName =  fullfile(directory,files(frame).name);
    data = load(fileName);
    particle = data.particle;
    particle = disksolve(particle, maskradius, scaling, files, frame,fileName, verbose, fitoptions);
end
