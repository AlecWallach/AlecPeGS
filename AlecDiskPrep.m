...
files = dir([directory, 'DSC03323.JPG']);
...
for frame = 1:nFrames %Loops for total number of images
    ...
    imageFilePrePro = [preProDirectory,files(frame).name];
    ...
    save([directory,'preprocessing files/', files(frame).name(1:end-4),'_preprocessing.mat'],'particle');
    %saveas(figure(1),[directory,'particle detection images/', files(frame).name(1:end-4),'.jpg'])
    end
